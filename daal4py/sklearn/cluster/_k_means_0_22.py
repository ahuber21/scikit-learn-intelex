...
import daal4py
from .._utils import getFPType, daal_check_version, PatchingConditionsChain
...
def _daal4py_k_means_fit(
    X, nClusters, numIterations, tol, cluster_centers_0, n_init, random_state
):
    if numIterations < 0:
...
def _fit(self, X, y=None, sample_weight=None):
    ...
    if self.precompute_distances == 'auto':
        ...
        raise ValueError(
            ...
            ", but a value of %r was passed" % self.precompute_distances
        )
    ...
    _dal_ready = _patching_status.and_conditions(
        [
            ...
            (
                not precompute_distances,
                "The precomputing of distances is not supported.",
            ),
        ]
    )
