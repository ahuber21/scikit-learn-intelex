...
class _BaseKMeans(TransformerMixin, ClusterMixin, ABC):
    ...
    def _init_centroids_onedal(
        ...
        if isinstance(init, str) and init == "k-means++":
            if not is_csr:
                ...
                # alg.compute.update_policy(queue, X)
            else:
                alg = self._get_kmeans_init(
                    cluster_count=n_clusters, seed=random_seed, algorithm="plus_plus_csr"
                )
                # alg.compute.update_policy(None, None)
            ...
            host_only = self.compute.backend_type == "host"
            centers = _convert_to_supported(host_only, queue, centers)
    ...
    def _init_centroids_sklearn(self, X, init, random_state, dtype=np.float32):
        ...
        host_only = True  # stock sklearn is host only
        queue = None
        centers = _convert_to_supported(host_only, queue, centers)
    ...
    def _fit(self, X, queue=None):
        is_csr = _is_csr(X)
        X = _check_array(
            X, dtype=[np.float64, np.float32], accept_sparse="csr", force_all_finite=False
        )
        host_only = self.train.backend_type == "host"
        X = _convert_to_supported(host_only, queue, X)
        dtype = get_dtype(X)
        X_table = to_table(X)
        ...
        for _ in range(self._n_init):
            ...
            self.train.update_policy(queue, X_table)
    ...
    def _predict(self, X, queue=None, result_options=None):
        is_csr = _is_csr(X)

        host_only = self.infer.backend_type == "host"
        X = _convert_to_supported(host_only, queue, X)
        X_table, dtype = to_table(X), X.dtype
        params = self._get_onedal_params(is_csr, dtype, result_options)

        self.infer.update_policy(queue, X)
